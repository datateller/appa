...
import net.ywb.app.bean.Update;
import net.ywb.app.bean.User;
import net.ywb.app.bean.UserInformation;
import net.ywb.app.bean.WellcomeImage;
...
public class ApiClient {
	...
	public static User login(AppContext appContext, String username, String pwd) throws AppException {
		...
		try{
			Status status = Status.parse(_post(appContext, loginurl, params, null));
			Log.d("login","return of login http req is : " + status.getStatus() + status.getResult().toString());
			User user = new User();
			user.setAccount(username);
			user.setPassword(pwd);
			user.setRememberMe(true);
			user.setUserName(username);
			user.setValidate(status);
			return user;
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
	 ...
	 * @throws AppException
	 */
	public static BabyInfo babyInfo(AppContext appContext, final int uid) throws AppException {
		try{
			String newUrl = _MakeURL(URLs.BABYINFO, new HashMap<String, Object>(){{
				put("uid", uid);
			}});
			return BabyInfo.parse(http_get(appContext, newUrl));
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	 ...
	 * @throws AppException
	 */
	public static String babyFace(AppContext appContext, final int uid) throws AppException {
		try{
...
//	}
	...
	public static UserInformation information(AppContext appContext, int uid, int hisuid, String hisname, int pageIndex, int pageSize) throws AppException {
		...
		try{
			return new UserInformation();//sg注释
	...
	public static void main(String[] arg){
		try {
			ApiClient.login(new AppContext(), "sg", "sg");
